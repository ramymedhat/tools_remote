...
public final class Chunker {
  ...
  private String identifier; // Used to identify the Chunker source in error messages.
  ...
  public String toString() {
    String size = String.format("<%d bytes>", digest.getSizeBytes());
    return identifier.isEmpty() ? size : size + ": " + identifier;
  }
  ...
  /**
   * Reads and returns the full underlying data string, not chunked. The Chunker is then reset.
   */
  public ByteString fullData()  throws IOException {
    maybeInitialize();
    ByteString result;
    if (digest.getSizeBytes() == 0) {
      result = ByteString.EMPTY;
    }
    result = ByteString.readFrom(data);
    reset();
    return result;
  }
  ...
  public static class Builder {
    ...
    public Builder setInput(Digest digest, File file) {
      Preconditions.checkState(inputStream == null);
      this.digest = digest;
      this.identifier = file.toString();
      inputStream =
          () -> {
            try {
    ...
    public Chunker build() {
      ...
      return new Chunker(inputStream, digest, chunkSize, digestUtil, identifier);
    }
  }
}
