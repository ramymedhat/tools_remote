...
void PathResolver::PlatformConvertToString(
  ...
  // TODO(arthurhsu): use Chrome base FilePath object, which has everything
...
// TODO(kinuko): This does similar path conversion to PlatformConvert inline.
...
string PathResolver::ResolvePath(absl::string_view path,
  ...
  if (sep_type == kPosixPathSep) {
    ...
    std::cerr << "Unknown sep_type=" << sep_type;
    return string(path);
  }
...
string PathResolver::WeakRelativePath(
  ...
  if (IsPosixAbsolutePath(raw_cwd)) {
    ...
    std::cerr << "Unknown path type given to raw_cwd=" << raw_cwd;
    return raw_path;
  }
  ...
  std::cerr << "expect real_cwd[0] == sep_type"
      << " real_cwd=" << real_cwd
      << " sep_type=" << sep_type;
