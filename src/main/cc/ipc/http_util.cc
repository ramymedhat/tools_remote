...
class Stream {
 public:
  ...
  Status ConsumePrefix(absl::string_view prefix) {
    ...
    if (buf.empty()) {
      //std::cerr << "not enough data to match:"
      //        << absl::CEscape(prefix);
  ...
  Status ConsumeUntil(absl::string_view needle) {
    ...
    do {
      ...
      if (buf.empty()) {
        //std::cerr << "not enough data to finding " << absl::CEscape(needle);
        return Status::ParseIncomplete;
      }
  ...
  Status ConsumeSize(size_t* size) {
    ...
    *size = 0;
    do {
      ...
      if (buf.empty()) {
        //std::cerr << "not enough data for size:"
 ...
 private:
  ...
  absl::string_view Ensure(size_t size) {
    ...
    //std::cerr << "need=" << size << " buf:" << CEscape(buf);
    if (size <= buf.size()) {
      //std::cerr << "buf:" << CEscape(buf);
      return buf;
    }
    size_t need = offset_ + size - non_chunk_data_->size();
    //std::cerr << "need=" << need << " input size:" << input_->size();
    if (need <= input_->size()) {
...
absl::string_view ExtractHeaderField(
  ...
  while (!header.empty()) {
    ...
    if (crlf == absl::string_view::npos) {
      // no end-of-header?
      std::cerr << "no end-of-header CRLFCRLF? "
                 << "finding " << field_name
...
//      std::cerr << "not match with " << field_name
//              << ": skip " << absl::CEscape(header.substr(0, crlf));
 ...
 //     std::cerr << "no colon after " << field_name
   //           << ": skip " << absl::CEscape(header.substr(0, crlf));
      header.remove_prefix(crlf + 2);
      continue;
    }
    //std::cerr << "found " << field_name << ": "
    ...
    //std::cerr << "following lines:" << absl::CEscape(rest);
    ...
    //std::cerr << "field value:" << absl::CEscape(field);
...
bool FindContentLengthAndBodyOffset(
  ...
  if (response_body == absl::string_view::npos) {
    std::cerr << "GOMA: Invalid, missing CRLFCRLF";
    return false;
  }
...
bool ParseHttpResponse(absl::string_view response,
  ...
  *http_status_code = atoi(string(codestr).c_str());
  ...
  //std::cerr << "HTTP header=" << response.substr(0, *offset);
  return true;
}
...
bool HttpChunkParser::Parse(absl::string_view input,
  ...
  done_ = false;
  //std::cerr << "parse chunk stream";
  while (!input.empty()) {
    if (last_chunk_remain_ > 0) {
    //  std::cerr << "last_chunk_remain=" << last_chunk_remain_;
    ...
    switch (s) {
      ...
      case Stream::Status::ParseIncomplete:
        //std::cerr << "need more data for CRLF at the end of chunk-data"
          //      << absl::CEscape(non_chunk_data_);
        return true;
    ...
    switch (s) {
      case Stream::Status::ParseError:
        return false;
      case Stream::Status::ParseIncomplete:
   //     std::cerr << "need more data for chunk-size:"
     //           << absl::CEscape(non_chunk_data_);
        return true;
      case Stream::Status::ParseOk:
        break;
   ...
   //   std::cerr << "skip trailer";
      s = stream.ConsumeUntil("\r\n\r\n");
      switch (s) {
        case Stream::Status::ParseError:
          std::cerr << "parse error to find CRLFCRLF?";
          exit(44);
        case Stream::Status::ParseIncomplete:
      //    std::cerr << "need more data for trailer:"
        //          << absl::CEscape(non_chunk_data_);
          return true;
        case Stream::Status::ParseOk:
          break;
      }
      //std::cerr << "all chunked-body received";
      done_ = true;
      return true;
    }
    // skip chunk-extension.
    //std::cerr << "skip chunk-extension";
    s = stream.ConsumeUntil("\r\n");
    switch (s) {
      case Stream::Status::ParseError:
        std::cerr << "parse error to find CRLF?";
        exit(44);
      case Stream::Status::ParseIncomplete:
   //     std::cerr << "need more data for chunk-extension:"
       //         << absl::CEscape(non_chunk_data_);
        return true;
      case Stream::Status::ParseOk:
        break;
  ...
  //  std::cerr << "next chunk-size=" << last_chunk_remain_;
  }
  //std::cerr << "no more data in buffer. need more data"
