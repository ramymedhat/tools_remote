...
public class ByteStreamUploaderTest {
  ...
  public final void setUp() throws Exception {
    ...
    server =
        InProcessServerBuilder.forName(serverName)
            ...
            .build()
            .start();
    channel = new ReferenceCountedChannel(InProcessChannelBuilder.forName(serverName).build());
    withEmptyMetadata =
            ...
            return new StreamObserver<WriteRequest>() {
              ...
              public void onNext(WriteRequest writeRequest) {
                if (nextOffset == 0) {
                  ...
                  assertThat(writeRequest.getResourceName()).startsWith("foo/uploads");
  ...
  public void multipleBlobsUploadShouldWork() throws Exception {
    ...
    ByteStreamUploader uploader =
        new ByteStreamUploader.Builder().setChannel(channel).setRetrier(retrier).build();
  ...
  @Test()
  public void simpleBatchUploadsShouldWork() throws Exception {
    Context prevContext = withEmptyMetadata.attach();
    RemoteRetrier retrier =
        new RemoteRetrier(
            () -> new FixedBackoff(10, 0), (e) -> true, retryService, Retrier.ALLOW_ALL_CALLS);
    ByteStreamUploader uploader =
        new ByteStreamUploader.Builder()
            .setInstanceName("abc")
            .setChannel(channel)
            .setRetrier(retrier)
            .setBatchMaxNumBlobs(10)
            .setBatchMaxSize(300)
            .build();
    ...
    chunkers.add(
        Chunker.builder(DIGEST_UTIL).setInput("blabla".getBytes(UTF_8)).setChunkSize(1).build());
    chunkers.add(
    ...
    chunkers.add(
        Chunker.builder(DIGEST_UTIL).setInput("bazz".getBytes(UTF_8)).setChunkSize(1).build());
    ...
    serviceRegistry.addService(
        new ContentAddressableStorageImplBase() {
          ...
          public void batchUpdateBlobs(
            ...
            numBlobs.addAndGet(request.getRequestsCount());
  ...
  @Test(timeout = 20000)
  public void multipleBlobsBatchUploadShouldWork() throws Exception {
    Context prevContext = withEmptyMetadata.attach();
    RemoteRetrier retrier =
        new RemoteRetrier(
            () -> new FixedBackoff(10, 0), (e) -> true, retryService, Retrier.ALLOW_ALL_CALLS);
    ByteStreamUploader uploader =
        new ByteStreamUploader.Builder()
            .setChannel(channel)
            .setRetrier(retrier)
            .setBatchMaxNumBlobs(100)
            .setBatchMaxSize(3000)
            .build();
            ...
            return new StreamObserver<WriteRequest>() {
              ...
              public void onNext(WriteRequest writeRequest) {
                if (writeRequest.getWriteOffset() == 0) {
                  ...
                  assertThat(components).hasLength(5);
                  String digestHash = components[3];
    ...
    blockUntilInternalStateConsistent(uploader);
    assertThat(numBlobs.get()).isEqualTo(blobsByHash.size());
  ...
  public void contextShouldBePreservedUponRetries() throws Exception {
          ...
          public StreamObserver<WriteRequest> write(StreamObserver<WriteResponse> response) {
            return new StreamObserver<WriteRequest>() {
              ...
              public void onNext(WriteRequest writeRequest) {
                ...
                assertThat(meta.getToolDetails().getToolName()).isEqualTo("remote_client");
                synchronized (this) {
    ...
    for (Chunker chunker : builders) {
      ...
      ctx.call(
          () -> {
            uploads.addAll(uploader.uploadBlobsAsync(
                ImmutableList.of(chunker), true, RunRecord.newBuilder()).values());
            return null;
          });
    }
  ...
  public void sameBlobShouldNotBeUploadedTwice() throws Exception {
    ...
    Future<?> upload1 = uploader.uploadBlobsAsync(
        ImmutableList.of(chunker), true, RunRecord.newBuilder()).get(digest);
    Future<?> upload2 = uploader.uploadBlobsAsync(
        ImmutableList.of(chunker), true, RunRecord.newBuilder()).get(digest);
  ...
  public void shutdownShouldCancelOngoingUploads() throws Exception {
    ...
    ListenableFuture<Void> f1 =
        uploader.uploadBlobsAsync(
            ImmutableList.of(chunker1), true, RunRecord.newBuilder()).get(chunker1.digest());
    ListenableFuture<Void> f2 =
        uploader.uploadBlobsAsync(
            ImmutableList.of(chunker2), true, RunRecord.newBuilder()).get(chunker2.digest());
      ...
      return new StreamObserver<WriteRequest>() {
        ...
        public void onNext(WriteRequest writeRequest) {
          if (nextOffset == 0) {
            ...
            String[] components = resourceName.split("/");
            assertThat(components).hasLength(5);
            digestHash = components[3];
            assertThat(blobsByHash).containsKey(digestHash);
            receivedData = new byte[Integer.parseInt(components[4])];
          }
          assertThat(digestHash).isNotNull();
